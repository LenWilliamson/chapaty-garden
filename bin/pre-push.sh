...
echo -e "\n${YELLOW}[2/11] Checking Formatting (Nightly)...${NC}"
...
echo -e "\n${YELLOW}[3/11] Checking Cargo.toml Sort Order...${NC}"
...
echo -e "\n${YELLOW}[4/11] Checking chapaty Dependency (No Local Path)...${NC}"
...
echo -e "\n${YELLOW}[5/11] Security Audit (Dependencies)...${NC}"
...
echo -e "\n${YELLOW}[6/11] Linting (Clippy)...${NC}"
cargo clippy --all-targets --all-features -- -D warnings
echo -e "${GREEN}[OK] Code is clean.${NC}"

echo -e "\n${YELLOW}[7/11] Building Workspace...${NC}"
cargo build --all-features
echo -e "${GREEN}[OK] Workspace compiled successfully.${NC}"

echo -e "\n${YELLOW}[8/11] Running Unit & Integration Tests...${NC}"
...
echo -e "\n${YELLOW}[10/11] Building Release Binary...${NC}"
cargo build --release
echo -e "${GREEN}[OK] Release binary compiled successfully.${NC}"

echo -e "\n${YELLOW}[11/11] Running Demo via 'make run' (Sanity Check)...${NC}"
